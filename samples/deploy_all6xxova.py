...
def setup_args():
    ...
    parser.add_argument('-defaultServerName', '--defaultServerName', required=True,
...
def main():
    ...
    if args.network:
        ...
        cisp = vim.OvfManager.CreateImportSpecParams(entityName=args.defaultServerName, diskProvisioning='thin',
        ...
        # networkMapping (to map to networks)
        # propertyMapping (descriptor specific properties)
        cisp = vim.OvfManager.CreateImportSpecParams(entityName=args.defaultServerName, diskProvisioning='thin')  # deploy disk type is thin, default is thick
        cisr = ovfManager.CreateImportSpec(ovf_handle.get_descriptor(),
                                           rp, ds, cisp)
    ...
    # # networkMapping (to map to networks)
    # # propertyMapping (descriptor specific properties)
    # cisp = vim.OvfManager.CreateImportSpecParams(entityName='AutomationServer620', diskProvisioning='thin') #deploy disk type is thin, default is thick
    # cisr = ovfManager.CreateImportSpec(ovf_handle.get_descriptor(),
    #                                    rp, ds, cisp)
...
class OvfHandler(object):
    ...
    def __init__(self, ovafile):
        ...
        self.descriptor = ovffile.read().decode('utf-8')
